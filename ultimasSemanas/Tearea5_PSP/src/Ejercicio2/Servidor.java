...
import Ejercicio1.Mensaje;
import Ejercicio1.Pagina;
...
private  Socket socketCliente;
    ...
    public void run() {
        try {
            ...
            this.socketCliente.close(); // Cerramos el socket al terminar la tarea
        } catch (IOException ex) {
            Logger.getLogger(Servidor.class.getName()).log(Level.SEVERE, null, ex);
        }/* finally {
            // CIERRE DE RECURSOS MANUAL
            try {
                if (printWriter != null) printWriter.close();
                if (bufLeer != null) bufLeer.close();
